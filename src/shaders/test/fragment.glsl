...
void main()
{
    ...
    // Pattern 8
    // float strength = mod(vUv.y * 10.0, 1.0);
    // strength = step(0.5, strength); // if strengtht is above 0.5 the step will provide 1.0 and if strengtht is below 0.5 step will provide 0.0
    ...
    // OR
    // float s1 = step(0.2, max(abs(vUv.x - 0.5), abs(vUv.y - 0.5)));
    // float s2 = 1.0 - step(0.25, max(abs(vUv.x - 0.5), abs(vUv.y - 0.5)));
    ...
    //    vUv.y * 0.5 + 0.25
    // );
    // float ligthX = 0.015 / distance(lightUvX, vec2(0.5, 0.5));
    ...
    //    vUv.x * 0.5 + 0.25
    // );
    // float ligthY = 0.015 / distance(lightUvY, vec2(0.5, 0.5));
    ...
    // Pattern 50
    float strength = step(0.8, sin(cnoise(vUv * 16.0) * 25.0 + vUtime * 4.));
    // gl_FragColor = vec4(strength, strength, strength, 1.0);
    
    // Colered version
    vec3 blackColor = vec3(0.05);
